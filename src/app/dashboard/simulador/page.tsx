...
export default function SimuladorPage() {
  ...
  const handleSubmitSimulacao = async (seguradoraEscolhida: string) => {
    try {
      ...
      const notasSimulacao = `Simulação Stitch: ${veiculo} (${ano}), Placa: ${placa}, Seguradora Preferida: ${seguradoraEscolhida}. FIPE: ${coberturaFipe ? 'Sim' : 'Não'}, Terceiros R$300k: ${danosTerceiros ? 'Sim' : 'Não'}, Carro Reserva 15d: ${carroReserva ? 'Sim' : 'Não'}`;
      ...
      const res = await fetch('/api/leads/intake', {
        ...
        body: JSON.stringify({
          nome,
          telefone,
          origem: `Simulador Stitch - ${seguradoraEscolhida}`,
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto pb-16">
      {/* Stitch Sync Banner */}
      <div className="bg-gradient-to-r from-blue-900/40 via-indigo-900/30 to-emerald-900/30 border border-blue-500/30 rounded-2xl p-4 sm:p-5 backdrop-blur-md flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-blue-600/30 text-blue-400 border border-blue-400/30 flex items-center justify-center text-xl shadow-inner">
            🎨
          </div>
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] text-emerald-400 font-semibold">Sincronizado via StitchMCP</span>
            </div>
            <h2 className="text-sm sm:text-base font-bold text-slate-100">
              Projeto Stitch: Insurance Lead Engine - Prototipagem
            </h2>
            <p className="text-xs text-slate-400">
              Projeto ID: <code className="text-slate-300 font-mono">9209791327557886092</code> — Criado e disponível na sua conta web em <a href="https://stitch.withgoogle.com" target="_blank" rel="noreferrer" className="text-blue-400 hover:underline">stitch.withgoogle.com</a>
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="px-4 py-2 rounded-xl bg-slate-800/80 hover:bg-slate-700 text-slate-200 border border-slate-700 text-xs font-semibold transition-all flex items-center gap-1.5 cursor-pointer"
          >
            <span>🌐</span> Abrir no Google Stitch
          </a>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-slate-900/70 border border-slate-800/80 rounded-2xl p-6 backdrop-blur-md shadow-xl">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 bg-slate-950 p-1.5 rounded-xl border border-slate-800 mb-6">
              {[
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-700/80 rounded-xl px-3.5 py-2 text-xs text-slate-100 font-mono uppercase focus:outline-none focus:border-blue-500"
                  />
                </div>
              </div>
              ...
              <div className="pt-3 border-t border-slate-800 space-y-3">
                ...
                <label className="flex items-center justify-between p-2.5 rounded-xl bg-slate-950/80 border border-slate-800 cursor-pointer hover:border-slate-700 transition-colors">
                ...
                <label className="flex items-center justify-between p-2.5 rounded-xl bg-slate-950/80 border border-slate-800 cursor-pointer hover:border-slate-700 transition-colors">
                  <div className="flex flex-col">
                    <span className="text-xs font-semibold text-slate-200">Danos Materiais/Corporais Terceiros</span>
                    <span className="text-[10px] text-slate-400">R$ 300.000 de indenização</span>
                  </div>
                  <input
                    type="checkbox"
                ...
                <label className="flex items-center justify-between p-2.5 rounded-xl bg-slate-950/80 border border-slate-800 cursor-pointer hover:border-slate-700 transition-colors">
                  <div className="flex flex-col">
                    <span className="text-xs font-semibold text-slate-200">Carro Reserva Plus</span>
                    <span className="text-[10px] text-slate-400">15 dias automático categoria Sedã Médio</span>
                  </div>
                  <input
                    type="checkbox"
                ...
                <label className="flex items-center justify-between p-2.5 rounded-xl bg-slate-950/80 border border-slate-800 cursor-pointer hover:border-slate-700 transition-colors">
                  <div className="flex flex-col">
                    ...
                    <span className="text-[10px] text-slate-400">Inclui película e teto solar</span>
                  </div>
                  <input
                    type="checkbox"
                ...
                <label className="flex items-center justify-between p-2.5 rounded-xl bg-slate-950/80 border border-slate-800 cursor-pointer hover:border-slate-700 transition-colors">
                  <div className="flex flex-col">
                    ...
                    <span className="text-[10px] text-slate-400">Em todo o território nacional</span>
                  </div>
                  <input
                    type="checkbox"
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-slate-400">3 seguradoras compatíveis</span>
          </div>

          {/* Card 1: Porto Seguro (Featured / Best Value) */}
          <div className="relative bg-gradient-to-b from-slate-900/90 to-slate-950 border-2 border-emerald-500/50 rounded-2xl p-6 shadow-2xl backdrop-blur-md overflow-hidden">
            <div className="absolute top-0 right-0 bg-emerald-500 text-slate-950 text-[10px] font-extrabold px-3 py-1 rounded-bl-xl uppercase tracking-wider shadow">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800/80 pb-4 mb-4">
              ...
              <div className="text-right">
                ...
                <span className="block text-[11px] text-slate-400">ou R$ 2.677,32 à vista (-10% PIX)</span>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full py-3.5 px-6 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-slate-950 font-extrabold text-sm shadow-lg shadow-emerald-600/30 transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
            >
          ...
          {/* Card 2: Tokio Marine */}
          <div className="bg-slate-900/60 border border-slate-800/80 rounded-2xl p-5 backdrop-blur-sm shadow-md">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-3 mb-3">
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-lg font-bold text-slate-100">Tokio Marine Seguradora</span>
              ...
              <div className="text-right">
                <span className="text-xl font-bold text-slate-200">12x R$ 229,15</span>
                <span className="block text-[10px] text-slate-400">R$ 2.474,80 à vista</span>
              </div>
            </div>
          ...
          <div className="bg-slate-900/60 border border-slate-800/80 rounded-2xl p-5 backdrop-blur-sm shadow-md">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-3 mb-3">
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-lg font-bold text-slate-100">Allianz Seguros</span>
              ...
              <div className="text-right">
                <span className="text-xl font-bold text-slate-200">12x R$ 265,40</span>
                <span className="block text-[10px] text-slate-400">R$ 2.866,32 à vista</span>
              </div>
            </div>
          ...
          <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 text-xs text-slate-400 flex items-center gap-3">
            ...
            <div>
              <strong className="text-slate-200 block">Garantia Speed-to-Lead Prime</strong>
              <span>
